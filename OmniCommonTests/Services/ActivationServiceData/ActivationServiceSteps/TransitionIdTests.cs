namespace OmniCommonTests.Services.ActivationServiceData.ActivationServiceSteps
{
    ...
    public class TransitionIdTests
    {
        ...
        public void Setup()
        {
            _stateId = 1;
            _state = 2;
            _subject = new TransitionId(this._stateId, _state);
        }

        [Test]
        public void Equals_SecondIdHasEqualSourceStepTypeAndState_ReturnsTrue()
        {
            _subject.Equals(new TransitionId(this._stateId, _state)).Should().BeTrue();
        }

        [Test]
        public void Equals_SecondIdHasEqualSourceStepTypeButDifferentState_ReturnsFalse()
        {
            _subject.Equals(new TransitionId(this._stateId, 5)).Should().BeFalse();
        }
